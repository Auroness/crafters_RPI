 //////////////////////////////////////////////////////////////////////////////
 //
 /// net_link.cpp - Data Structures for network socket access
 //
 //
 /// Crafters RPI
...
new_descriptor (int s)
{
	...
	*ip = '\0';
	...
	if (!descriptor_list)
        ...
        char echo_on_str[] = { (char) IAC, (char) WONT, (char) TELOPT_ECHO, '\0' };

        SEND_TO_Q (echo_on_str, newd);
...
init_descriptor (DESCRIPTOR_DATA* newd, int desc)
{
	...
	newd->hSocketFD = desc;
	newd->strClientHostname = NULL;
	newd->strClientIpAddr = NULL;
...
{
	char dummy[MAX_STRING_LENGTH]= { '\0' };
...
init_socket (int port_num)
{
  ...
  extern int setsockopt(int s, int level, int optname, char *optval, int optlen);
	...
	if ((run_sock = socket (AF_INET, SOCK_STREAM, 0)) < 0)
	{
		perror ("init_socket: socket()");
		exit (1);
	}
	opt = 1;
	if (setsockopt (run_sock, SOL_SOCKET, SO_REUSEADDR, (char *) &opt, sizeof (opt)) <
		0)
	{
	...
	if (setsockopt (run_sock, SOL_SOCKET, SO_LINGER, (char *) &ld, sizeof (ld)) < 0)
	{
		perror ("setsockopt");
		exit (1);
	}
	
	if (bind (run_sock, (struct sockaddr *) &sa, sizeof (sa)) < 0)
	{
...
close_socket (DESCRIPTOR_DATA * d)
{
	...
	else if (d->character && (d->connected == CON_PLYNG))
	{
		...
		{
			(d->character)->act("$n has lost link.", true, 0, 0, TO_ROOM);
			
			sprintf (buf, "Closing link to: %s.", (d->character)->name);
...
{
	char toutput[MAX_STRING_LENGTH + 1];
	...
	while (get_from_q (&t->output, toutput))
	{
		if (t->snoop.snoop_by && t->snoop.snoop_by->desc != NULL
			&& !IS_NPC (t->snoop.snoop_by))
		{
			write_to_q ("% ", &t->snoop.snoop_by->desc->output);
			write_to_q (toutput, &t->snoop.snoop_by->desc->output);
		}
		if (write_to_descriptor (t, toutput))
			return (-1);
	}
