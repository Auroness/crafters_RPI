//////////////////////////////////////////////////////////////////////////////
//
/// create_mobile.cpp - Mobile Autocreation Module
//
//
/// Crafters RPI
...
{
	int roll, limit;
	static char adj[MAX_STRING_LENGTH]= { '\0' };
...
return_name (CHAR_DATA * mob)
{
	...
	*buf = '\0';
		...
		"Vanya Elf"))
	{
		...
		if (roll == 0 && IS_NPC(mob) && !IS_SET (mob->mob->profession, PROF_ENFORCER))
			sprintf (buf, "old ");
		else if (roll == 4 && IS_NPC(mob) && !IS_SET (mob->mob->profession, PROF_ENFORCER))
			sprintf (buf, "young ");
		...
		if (IS_NPC(mob) && IS_SET (mob->mob->profession, PROF_ENFORCER))
		{
			if (mob->sex == SEX_MALE)
				sprintf (buf, "man");
			else
...
create_description (CHAR_DATA * mob)
{
	...
	*buf = '\0';
	if (IS_NPC(mob) 
		&& IS_SET (mob->mob->profession, PROF_ENFORCER)
		&& IS_SET (mob->mob->profession, PROF_SENTINEL))
	{
		roll = number (1, 3);
		if (roll == 1)
	...
	else if (!str_cmp (lookup_race_variable (mob->race, RACE_NAME), "Ent"))
	{
		...
		else if (!str_cmp (lookup_race_variable (mob->race, RACE_NAME), "Uruk-Hai"))
		{
			...
			*buf = toupper (*buf);
			...
			if (mob->keywords)
				free_mem (mob->keywords);
			mob->keywords = duplicateString (buf2);
}
...
randomize_mobile (CHAR_DATA * mob)
{
	CHAR_DATA *proto;
	int attr_starters[8];
	int attr_modifiers[8]; 
	int attr_startersUnset[8];
	int i, roll;
	int type_bonus = 0;
	...
	//Fill an array with the stat values from the prototype
	attr_starters[0] = mob->str;
	attr_starters[1] = mob->dex;
	attr_starters[2] = mob->con;
	attr_starters[3] = mob->wil;
	attr_starters[4] = mob->intel;
	attr_starters[5] = mob->agi;
	attr_starters[6] = mob->aur;
	attr_starters[7] = mob->luk; 

		//Fill an array with the stat modifiers for the mob's race
	attr_modifiers[0] = atoi(lookup_race_variable(mob->race, RACE_STR_MOD));
	attr_modifiers[1] = atoi(lookup_race_variable(mob->race, RACE_DEX_MOD));
	attr_modifiers[2] = atoi(lookup_race_variable(mob->race, RACE_CON_MOD));
	attr_modifiers[3] = atoi(lookup_race_variable(mob->race, RACE_WIL_MOD));
	attr_modifiers[4] = atoi(lookup_race_variable(mob->race, RACE_INT_MOD));
	attr_modifiers[5] = atoi(lookup_race_variable(mob->race, RACE_AUR_MOD));
	attr_modifiers[6] = atoi(lookup_race_variable(mob->race, RACE_AGI_MOD));
	attr_modifiers[7] = 0;  //luck is not racially modified
	
		//Starters if total begins lower than 82
	attr_startersUnset[0] = 15;
	attr_startersUnset[1] = 13;
	attr_startersUnset[2] = 11;
	attr_startersUnset[3] = 11;
	attr_startersUnset[4] = 10;
	attr_startersUnset[5] = 8;
	attr_startersUnset[6] = 3;
	attr_startersUnset[7] = 3; 
	
	if (is_name_in_list("legendary", mob->keywords)) {
		type_bonus = 50;
	}

	else if (is_name_in_list("epic", mob->keywords)) {
		type_bonus = 40;
	}

	else if (is_name_in_list("elite", mob->keywords)) {
		type_bonus = 30;
	}

	else if (is_name_in_list("plebe", mob->keywords)) {
		type_bonus = 10;
	}
	...
	if (IS_SET (mob->flags, FLAG_GUEST))
	{
		...
		mob->aur = (attr_starters[6] > 1 ? number(1, mob->aur)/4 : 1); 
		mob->luk = attr_starters[7];
		...
		mob->skill_map.clear();
		mob->char_clan_map.clear();
		
		if (IS_NPC(mob) && IS_SET (mob->mob->profession, PROF_ENFORCER))
			mob->skill_map["Subdue"] = 30 + number (1, 10) + type_bonus;


		mob->skill_map["Scan"] = 10 + number (40, 60) + type_bonus;
		mob->skill_map["Listen"] = 50 + number (1, 40) + type_bonus;
		mob->skill_map["Hide"] = 30 + number (1, 30) + type_bonus;
		mob->skill_map["Sneak"] = 30 + number (1, 30) + type_bonus;

		mob->skill_map["Climb"] = 10 + number (5, 15) + type_bonus;
		mob->skill_map["Search"] = 10 + number (5, 15) + type_bonus;

		expose_skill(mob, lookup_skill_id("Barter"));
				
		make_height (mob);
		make_frame (mob);

		//TODO: add a bit to all skills
		/***
		for (i = 1; i <= LAST_SKILL; i++)
		{
			if (IS_NPC(mob) && IS_SET (mob->mob->profession, PROF_ENFORCER) && i == SKILL_SUBDUE)
				continue;
		...
		{
			// TODO: change this to a better proto mob
			proto = vtom (VNUM_BASE_MOBILE);
		}

		if (lookup_race_variable (mob->race, RACE_NATIVE_TONGUE))
		{
			// ignore common humans whose native tongue variable is not accurate, since it's location dependent
			mob->speaks = lookup_skill_name(atoi (lookup_race_variable (mob->race, RACE_NATIVE_TONGUE)));
			if (mob->race!=0) 
			{
				mob->skill_map[mob->speaks] = calc_lookup(mob, REG_CAP, lookup_skill_id(mob->speaks));
			}
		}

		// ensure Westron for NPCs 
		if ((mob->skill_map["Westron"] <= 0) && IS_NPC(mob))
			mob->skill_map["Westron"] = 20;

			//if we have a prototype, add in the extra skills if needed
		
		if (proto)
		{
			/*** //TODO: add in proper skills
			for (i = 1; i <= LAST_SKILL; i++)
			{
				if (mob->skills[i] == 0)
			...
			***/
			mob->speaks = strdup(proto->speaks);
		}
		else 
		{
			mob->speaks = strdup("Westron");	
		}
